...
int PbEach::xConnect(sqlite3 *db, void *pAux, int argc, const char * const *argv, sqlite3_vtab **ppVtab, char **pzErr){
    sqlite3_vtab *pNew ;
    int rc;

#define _PB_EACH_COLUMN_VALUE 0
#define _PB_EACH_COLUMN_REPEATED_ID 1
#define _PB_EACH_COLUMN_NAME 2
#define _PB_EACH_COLUMN_NUMBER 3
#define _PB_EACH_COLUMN_LABEL 4
#define _PB_EACH_COLUMN_TYPE 5
#define _PB_EACH_COLUMN_TYPE_NAME 6
#define _PB_EACH_COLUMN_MESSAGE 7
#define _PB_EACH_COLUMN_MESSAGE_TYPE 8
#define _PB_EACH_COLUMN_FIELD_PATH 9

// equivalent the MESSAGE
#define _PB_EACH_COLUMN_M 10

// equivalent the MESSAGE_TYPE
#define _PB_EACH_COLUMN_MT 11

// equivalent the FIELD_PATH
#define _PB_EACH_COLUMN_FP 12


// params

#define _PB_EACH_PARAM_FLAG_NONE 0x0
#define _PB_EACH_PARAM_FLAG_MESSAGE 0x1
#define _PB_EACH_PARAM_FLAG_MESSGE_TYPE_NAME 0x2
#define _PB_EACH_PARAM_FLAG_FIELD_PATH 0x4
...
int PbEach::xBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo){
    int rc = SQLITE_OK;
    int nArg = 0;          // Number of arguments
    int required_param_flags = _PB_EACH_PARAM_FLAG_NONE;  // Bit field to track the presence of required parameters
    int argIndex[4];
    ...
    const struct sqlite3_index_info::sqlite3_index_constraint *constraint = pIdxInfo->aConstraint;
    for(int i = 0; i < pIdxInfo->nConstraint; i ++, constraint ++) {
        if (constraint->op == SQLITE_INDEX_CONSTRAINT_EQ) {
            //            Set required_param_flags
            switch(constraint->iColumn) {
            case _PB_EACH_COLUMN_MESSAGE:
            case _PB_EACH_COLUMN_M:
                argIndex[0] = i;
                required_param_flags |= _PB_EACH_PARAM_FLAG_MESSAGE;
                break;
            case _PB_EACH_COLUMN_MESSAGE_TYPE:
            case _PB_EACH_COLUMN_MT:
                argIndex[1] = i;
                required_param_flags |= _PB_EACH_PARAM_FLAG_MESSGE_TYPE_NAME;
                break;
            case _PB_EACH_COLUMN_FIELD_PATH:
            case _PB_EACH_COLUMN_FP:
                argIndex[2] = i;
                required_param_flags |= _PB_EACH_PARAM_FLAG_FIELD_PATH;
                break;
            }
        }
    }
    ...
    if ((required_param_flags & 7) != 7){
        tab->zErrMsg = sqlite3_mprintf
            ("missing argument(s):%s %s %s",
             !(required_param_flags & _PB_EACH_PARAM_FLAG_MESSAGE) ? "'message'" : "",
             !(required_param_flags & _PB_EACH_PARAM_FLAG_MESSGE_TYPE_NAME) ? "'message_type'":"",
             !(required_param_flags & _PB_EACH_PARAM_FLAG_FIELD_PATH) ? "'field'" : ""
             );
        return SQLITE_ERROR;
    }
    for(int i= 0; i < 4; i++){
...
int PbEach::xFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum, const char *idxStr, int argc, sqlite3_value **argv){
    ...
    if (value_type == SQLITE_BLOB ){
        ...
        if(message != nullptr){
            ...
            pVtabCursor->pVtab->zErrMsg = sqlite3_mprintf("45re8wq6 pointer_init_error, not valid message_type:'%s'",message_type.c_str());
            return SQLITE_ERROR;
        }
    }
    ...
    if (field.find('.') != std::string::npos) {
        ...
        //init fieldInfo returned repeated_size xEof needs
        int rc = PbExtract::extract(nullptr, static_cast<google::protobuf::Message*>(const_cast<void*>(pCur->messagePtr)), pCur->message_type, pCur->field_name, &pCur->field_info);
        if (rc == SQLITE_OK){
            return SQLITE_OK;
        }else{
            pCur->base.pVtab->zErrMsg = sqlite3_mprintf(pCur->field_info.err.c_str());
            return SQLITE_ERROR;
        }
    }
...
int PbEach::xColumn(sqlite3_vtab_cursor *cur, sqlite3_context *ctx, int i){
    x_cursor *pCur = (x_cursor*)cur;
    FieldInfo &field_info = pCur->field_info;
    int rc = SQLITE_OK;
    switch( i ){
    case _PB_EACH_COLUMN_VALUE:{
        if(pCur->field_name.length()){
            std::ostringstream oss;
            oss << pCur->field_name << "(" << pCur->iRowid << ")";
            std::string field = oss.str();
            rc = PbExtract::extract(ctx, static_cast<google::protobuf::Message*>(const_cast<void*>(pCur->messagePtr)), pCur->message_type, field, &pCur->field_info);
        }
        break;
    }
    case _PB_EACH_COLUMN_NAME :
        sqlite3_result_text(ctx, field_info.name.c_str(),field_info.name.length(),SQLITE_TRANSIENT);
        break;
    case _PB_EACH_COLUMN_NUMBER :
        sqlite3_result_int(ctx, field_info.number);
        break;
    case _PB_EACH_COLUMN_LABEL :
        sqlite3_result_text(ctx, field_info.label.c_str(),-1,SQLITE_TRANSIENT);
        break;
    case _PB_EACH_COLUMN_TYPE :
        sqlite3_result_text(ctx, field_info.type.c_str(),-1,SQLITE_TRANSIENT);
        break;
    case _PB_EACH_COLUMN_TYPE_NAME :
        sqlite3_result_text(ctx, field_info.type_name.c_str(),-1,SQLITE_TRANSIENT);
        break;
    case _PB_EACH_COLUMN_REPEATED_ID :
        if(pCur->field_info.repeated){
